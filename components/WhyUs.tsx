        ...
        <div className="reveal-group grid grid-cols-1 gap-x-12 gap-y-12 md:grid-cols-2 md:gap-x-[110px] md:gap-y-[76px] lg:grid-cols-3">
          {REASONS.map(({ icon, title, desc }) => (
            <div key={title} className="reveal-item max-md:flex max-md:flex-col max-md:items-center max-md:text-center">
              <div className="mb-5 flex h-[102px] w-[102px] max-md:h-[80px] max-md:w-[80px] items-center justify-center rounded-2xl overflow-hidden">
                <Image src={icon} alt={title} width={80} height={80} className="object-contain w-full h-full" />
              </div>
              <h3 className="mb-3 text-[17px] max-md:text-[21px] font-semibold tracking-tight text-[#1d1d1d] font-display">{title}</h3>
              <p
                className="text-[17px] leading-normal tracking-tight text-[#86868B] font-display font-[500] md:max-w-2xl"
                style={{
