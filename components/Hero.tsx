...
export default function Hero() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // ── 1. Shrink (0 → vh×2.5), blur clears fast on first scroll ──
      const shrinkP = c(scrollY / (vh * 2.5));
      const scale1 = 0.9 - shrinkP * 0.72; // 0.90 → 0.18
      const blur = 16 * (1 - c(scrollY / (vh * 0.5))); // clears by 0.5vh

      // ── 2. Land on tablet (vh×2.5 → vh×4.5) ──────────────────────
      //    scale continues 0.18→0.07, blob moves from centre to tablet
      const landP = c((scrollY - vh * 2.5) / (vh * 2.0));
      ...
      // blob natural centre = vh/2 (because top:50% + translateY(-50%))
      const tyAtTablet = logoTargetY - vh * 0.5;

      const finalScale = (1 - landEt) * scale1 + landEt * 0.07;
      const finalTY = landEt * tyAtTablet;
      ...
      // ── 3. "Outstanding" gradient fade-in — after it clears the blob ─
      //    Starts at 0; fades in from vh×1.0 → vh×1.25 (after the text
      //    has scrolled above the shrinking blob circles)
      if (outstandingRef.current) {
        const outP = c((scrollY - vh * 1.0) / (vh * 0.25));
        outstandingRef.current.style.opacity = String(easeInOut(outP));
      }

      // ── 4. Brand mockup items rise — easeInOut (vh×2.0 → vh×4.5) ──
      if (brandRef.current) {
        const brandP = c((scrollY - vh * 2.0) / (vh * 2.5));
        brandRef.current
  ...
  return (
    /*
      ...
      - Sticky background panel (z-5): blob + brand grid
      - Scrolling text siblings (absolute within scroll container):
          "Branding"     top:12vh   z-10  — visible at scroll=0, scrolls off naturally
          "Let's make it" top:118vh  z-10  — scrolls in from below, sits ABOVE blob
          "Outstanding"  top:126vh  z-1   — scrolls through the blob circles (hidden
                                            behind them), then emerges colorful below
      550vh total; max scroll ≈ 4.5vh
    */
    <div style={{ height: "550vh", position: "relative" }}>
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            left: "50%",
            transform:
              "translateX(calc(-50% - 22px)) translateY(-50%) scale(0.9)",
      ...
      {/* ── "Branding" heading — z-10, scrolls naturally ─────────── */}
      {/*    visible at scroll=0, moves off-screen as user scrolls      */}
      <div
        className="pointer-events-none absolute inset-x-0 text-center"
        style={{ top: "12vh", zIndex: 10 }}
      >
        <h1
